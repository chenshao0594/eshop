...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/tax-class'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/tax-class', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: TaxClass) {
        return item.id;
    }
    registerChangeInTaxClasses() {
        this.eventSubscriber = this.eventManager.subscribe('taxClassListModification', (response) => this.loadAll());
    }
