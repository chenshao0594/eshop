...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.productOptionService.search({
    ...
    transition() {
        this.router.navigate(['/product-option'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/product-option', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ProductOption) {
        return item.id;
    }
