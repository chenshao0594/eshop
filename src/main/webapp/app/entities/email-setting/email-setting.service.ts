...
export class EmailSettingService {
    ...
    private resourceUrl = 'api/email-settings';
    private resourceSearchUrl = 'api/email-settings/_search';

    constructor(private http: Http, private dateUtils: DateUtils) { }

    create(emailSetting: EmailSetting): Observable<EmailSetting> {
        const copy: EmailSetting = Object.assign({}, emailSetting);
        copy.created_date = this.dateUtils
            .convertLocalDateToServer(emailSetting.created_date);
        copy.last_modified_date = this.dateUtils
            .convertLocalDateToServer(emailSetting.last_modified_date);
        return this.http.post(this.resourceUrl, copy).map((res: Response) => {
            return res.json();
        });
    }

    update(emailSetting: EmailSetting): Observable<EmailSetting> {
        const copy: EmailSetting = Object.assign({}, emailSetting);
        copy.created_date = this.dateUtils
            .convertLocalDateToServer(emailSetting.created_date);
        copy.last_modified_date = this.dateUtils
    ...
    find(id: number): Observable<EmailSetting> {
