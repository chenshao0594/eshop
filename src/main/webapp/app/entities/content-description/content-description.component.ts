...
@Component({
    selector: 'jhi-content-description',
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.contentDescriptionService.search({
        ...
        this.contentDescriptionService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: Response) => this.onSuccess(res.json(), res.headers),
    ...
    transition() {
        this.router.navigate(['/content-description'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ContentDescription) {
        return item.id;
    }
    registerChangeInContentDescriptions() {
        this.eventSubscriber = this.eventManager.subscribe('contentDescriptionListModification', (response) => this.loadAll());
    }
