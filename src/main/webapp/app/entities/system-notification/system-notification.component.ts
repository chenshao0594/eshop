...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.systemNotificationService.search({
        ...
        this.systemNotificationService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: Response) => this.onSuccess(res.json(), res.headers),
    ...
    transition() {
        this.router.navigate(['/system-notification'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: SystemNotification) {
        return item.id;
    }
    registerChangeInSystemNotifications() {
        this.eventSubscriber = this.eventManager.subscribe('systemNotificationListModification', (response) => this.loadAll());
    }
