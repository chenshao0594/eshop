...
export class MerchantLogService {
    ...
    private resourceUrl = 'api/merchant-logs';
    private resourceSearchUrl = 'api/merchant-logs/_search';

    constructor(private http: Http, private dateUtils: DateUtils) { }

    create(merchantLog: MerchantLog): Observable<MerchantLog> {
        const copy: MerchantLog = Object.assign({}, merchantLog);
        copy.created_date = this.dateUtils
            .convertLocalDateToServer(merchantLog.created_date);
        copy.last_modified_date = this.dateUtils
            .convertLocalDateToServer(merchantLog.last_modified_date);
        return this.http.post(this.resourceUrl, copy).map((res: Response) => {
            return res.json();
        });
    }

    update(merchantLog: MerchantLog): Observable<MerchantLog> {
        const copy: MerchantLog = Object.assign({}, merchantLog);
        copy.created_date = this.dateUtils
            .convertLocalDateToServer(merchantLog.created_date);
        copy.last_modified_date = this.dateUtils
    ...
    find(id: number): Observable<MerchantLog> {
