...
@Component({
    selector: 'jhi-order-account',
    templateUrl: './order-account.component.html'
})
export class OrderAccountComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.orderAccountService.search({
        ...
        this.orderAccountService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: Response) => this.onSuccess(res.json(), res.headers),
    ...
    transition() {
        this.router.navigate(['/order-account'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: OrderAccount) {
        return item.id;
    }
    registerChangeInOrderAccounts() {
        this.eventSubscriber = this.eventManager.subscribe('orderAccountListModification', (response) => this.loadAll());
    }
