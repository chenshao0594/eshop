...
@Component({
    selector: 'jhi-category-description',
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.categoryDescriptionService.search({
        ...
        this.categoryDescriptionService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: Response) => this.onSuccess(res.json(), res.headers),
    ...
    transition() {
        this.router.navigate(['/category-description'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/category-description', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: CategoryDescription) {
        return item.id;
    }
    registerChangeInCategoryDescriptions() {
        this.eventSubscriber = this.eventManager.subscribe('categoryDescriptionListModification', (response) => this.loadAll());
    }
    ...
    private onSuccess(data, headers) {
        ...
        this.categoryDescriptions = data;
    }
