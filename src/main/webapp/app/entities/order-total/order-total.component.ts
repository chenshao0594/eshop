...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/order-total'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/order-total', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OrderTotal) {
        return item.id;
    }
    registerChangeInOrderTotals() {
        this.eventSubscriber = this.eventManager.subscribe('orderTotalListModification', (response) => this.loadAll());
    }
