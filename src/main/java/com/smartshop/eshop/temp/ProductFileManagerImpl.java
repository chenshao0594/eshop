...
import com.smartshop.eshop.utils.ProductImageCropUtils;
import com.smartshop.eshop.utils.ProductImageSizeUtils;
...
public class ProductFileManagerImpl extends ProductFileManager {
	...
	public void addProductImage(ProductImage productImage, ImageContentFile contentImage) throws BusinessException {
		...
		try {
			 ...
			 * StringBuilder().append("L-").append(productImage.getProductImage(
			 * )).toString());
			 * 
			 * 
			 ...
			 * StringBuilder().append("S-").append(productImage.getProductImage(
			 * )).toString());
			 * 
			 * uploadImage.uploadProductImage(configuration, productImage,
			 * smallContentImage);
			...
			// String ssmallImageHeight =
			// configuration.getProperty("SMALL_IMAGE_HEIGHT_SIZE");
			// String ssmallImageWidth =
			// configuration.getProperty("SMALL_IMAGE_WIDTH_SIZE");
			...
			if (!StringUtils.isBlank(slargeImageHeight) && !StringUtils.isBlank(slargeImageWidth)) { // &&
				...
				if (largeImageHeight <= 0 || largeImageWidth <= 0) {
					...
					throw new BusinessException(sizeMsg);
				}
				...
				// do not keep a large image for now, just take care of the
				// regular image and a small image
				 ...
				 * ProductImageSizeUtils.resize(cropped, smallImageWidth,
				 * smallImageHeight); File tempSmall = File.createTempFile(new
				 * StringBuilder().append(productImage.getProduct().getId()).
				 * append("tmpSmall").toString(), "." + extension );
				 * ImageIO.write(smallResizedImage, extension, tempSmall);
				 * 
				 * //byte[] is = IOUtils.toByteArray(new
				 * FileInputStream(tempSmall));
				 * 
				 * FileInputStream isSmall = new FileInputStream(tempSmall);
				 * 
				 * output = new ByteArrayOutputStream(); IOUtils.copy(isSmall,
				 * output);
				 * 
				 * 
				 * smallContentImage = new
				 * InputContentImage(ImageContentType.PRODUCT);
				 * smallContentImage.setFile(output);
				 * smallContentImage.setDefaultImage(false);
				 * smallContentImage.setImageName(new
				 * StringBuilder().append("S-").append(productImage.
				 * getProductImage()).toString());
				 * 
				 * uploadImage.uploadProductImage(configuration, productImage,
				 * smallContentImage);
				 * 
	...
	@Override
	public OutputContentFile getProductImage(String merchantStoreCode, String productCode, String imageName,
			ProductImageSize size) throws BusinessException {
