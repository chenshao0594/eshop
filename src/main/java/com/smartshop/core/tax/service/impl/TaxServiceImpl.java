package com.smartshop.core.tax.service.impl;
...
import com.smartshop.core.tax.model.TaxBasisCalculation;
...
public class TaxServiceImpl implements TaxService {
	...
	@Override
	public TaxConfiguration getTaxConfiguration(MerchantStore store) throws BusinessException {

		MerchantConfiguration configuration = merchantConfigurationService.getMerchantConfiguration(TAX_CONFIGURATION,
				store);
	...
	@Override
	public List<TaxItem> calculateTax(SalesOrderSummary orderSummary, Customer customer, MerchantStore store,
		...
		if (!taxConfiguration.isCollectTaxIfDifferentProvinceOfStoreCountry()) {
			if ((zone != null && store.getZone() != null)
					&& (zone.getId().longValue() != store.getZone().getId().longValue())) {
				return null;
			}
		...
		// shippingService.getShippingConfiguration(store);
		...
		for (Long taxClassId : taxClassAmountMap.keySet()) {
			...
			if (!StringUtils.isBlank(stateProvince) && zone == null) {
				...
				taxRates = taxRateService.listByCountryZoneAndTaxClass(country, zone, taxClasses.get(taxClassId), store,
						language);
			}
			...
			for (TaxRate taxRate : taxRates) {
				...
				double taxRateDouble = taxRate.getTaxRate().doubleValue();// 5%
																			// ...
																			// 8%
																			// ...
