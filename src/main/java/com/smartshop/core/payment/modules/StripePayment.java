...
public class StripePayment implements PaymentModule {
	...
	public Transaction authorize(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
		...
		try {
			...
			transaction.setTransactionType(TransactionEnum.AUTHORIZE);
	...
	public Transaction capture(MerchantStore store, Customer customer, SalesOrder order,
		...
		try {
			...
			transaction.setOrder(order);
			transaction.setTransactionDate(new Date());
			transaction.setTransactionType(TransactionEnum.CAPTURE);
			transaction.setPaymentType(PaymentEnum.CREDITCARD);
	...
	private IntegrationException buildException(Exception ex) {
		...
		if (ex instanceof CardException) {
			 ...
			 * invalid_cvc The card's security code is invalid. incorrect_number
			 * The card number is incorrect. expired_card The card has expired.
			 * incorrect_cvc The card's security code is incorrect.
			 * incorrect_zip The card's zip code failed validation.
			 * card_declined The card was declined. missing There is no card on
			 * a customer that is being charged. processing_error An error
			 * occurred while processing the card. rate_limit An error occurred
			 * due to requests hitting the API too quickly. Please let us know
			 * if you're consistently running into this error.
			 */
