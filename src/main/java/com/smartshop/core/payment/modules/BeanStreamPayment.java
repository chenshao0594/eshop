...
import com.smartshop.eshop.utils.CreditCardUtils;
...
public class BeanStreamPayment implements PaymentModule {
	...
	public Transaction authorize(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
		...
		return processTransaction(store, customer, TransactionEnum.AUTHORIZE, amount, payment, configuration, module);
	}
	...
	public Transaction capture(MerchantStore store, Customer customer, SalesOrder order,
		...
		try {
			 ...
			 * &trnType=PAC&username=user1234&password=pass1234&trnID=1000 2115
			 * --> requires also adjId [not documented]
			 **/
			...
			Transaction response = this.sendTransaction(null, store, messageString.toString(), "PAC",
					TransactionEnum.CAPTURE, PaymentEnum.CREDITCARD, order.getTotal(), configuration, module);
	...
	public Transaction authorizeAndCapture(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
			BigDecimal amount, PaymentDTO payment, IntegrationConfiguration configuration, IntegrationModule module)
			throws IntegrationException {
		return processTransaction(store, customer, TransactionEnum.AUTHORIZECAPTURE, amount, payment, configuration,
				module);
	}
	...
	public Transaction refund(boolean partial, MerchantStore store, Transaction transaction, SalesOrder order,
		...
		try {
			...
			server = new StringBuffer().append(
					...
					configs.getScheme()).append("://").append(configs.getHost()).append(":").append(configs.getPort())
			 ...
			 * &trnOrderNumber=1234&trnAmount=1.00&adjId=1000 2115
			 **/
			StringBuilder messageString = new StringBuilder();
			...
			messageString.append("username=").append(configuration.getIntegrationKeys().get("username")).append("&");
			messageString.append("password=").append(configuration.getIntegrationKeys().get("password")).append("&");
			messageString.append("trnOrderNumber=").append(transaction.getTransactionDetails().get("TRNORDERNUMBER"))
					.append("&");
			messageString.append("trnAmount=").append(amnt).append("&");
			...
			Transaction response = this.sendTransaction(null, store, messageString.toString(), "R",
					TransactionEnum.REFUND, PaymentEnum.CREDITCARD, amount, configuration, module);
	...
	private Transaction sendTransaction(String orderNumber, MerchantStore store, String transaction,
			String beanstreamType, TransactionEnum transactionType, PaymentEnum paymentType, BigDecimal amount,
			IntegrationConfiguration configuration, IntegrationModule module) throws IntegrationException {
		...
		try {
			...
			// transaction =
			// "requestType=BACKEND&merchant_id=300200260&trnType=P&username=carlito&password=shopizer001&orderNumber=caa71106-7e3f-4975-a657-a35904dc32a0&trnCardOwner=Carl
			// Samson&trnCardNumber=5100000020002000&trnExpMonth=10&trnExpYear=14&trnCardCvd=123&trnAmount=77.01&ordName=Carl
			// S&ordAddress1=358 Du
			// Languedoc&ordCity=Victoria&ordProvince=BC&ordPostalCode=V8T2E7&ordCountry=CA&ordPhoneNumber=(444)
			// 555-6666&ordEmailAddress=csamson777@yahoo.com";
			/**
			 * requestType=BACKEND&merchant_id=300200260 &trnType=P
			 * &username=carlito&password=shopizer001
			 * &orderNumber=caa71106-7e3f-4975-a657-a35904dc32a0
			 * &trnCardOwner=Carl Samson &trnCardNumber=5100000020002000
			 * &trnExpMonth=10 &trnExpYear=14 &trnCardCvd=123 &trnAmount=77.01
			 * &ordName=Carl S &ordAddress1=378 Du Languedoc
			 * &ordCity=Boucherville &ordProvince=QC &ordPostalCode=J4B8J9
			 * &ordCountry=CA &ordPhoneNumber=(444) 555-6666
			 ...
			 * &trnCardNumber=4030000010001234 &trnExpMonth=10 &trnExpYear=16
			 * &ordName=Joe+Test &ordAddress1=123+Test+Street &ordCity=Victoria
			 * &ordProvince=BC &ordCountry=CA &ordPostalCode=V8T2E7
			 * &ordPhoneNumber=5555555555 &ordEmailAddress=joe%40testemail.com
			 **/
	...
	private Transaction processTransaction(MerchantStore store, Customer customer, TransactionEnum type,
			BigDecimal amount, PaymentDTO payment, IntegrationConfiguration configuration, IntegrationModule module)
			throws IntegrationException {
		...
		try {
			...
			if (type == TransactionEnum.AUTHORIZE) {
				transactionType = "PA";
			} else if (type == TransactionEnum.AUTHORIZECAPTURE) {
				transactionType = "P";
			}
			 ...
			 *
			 * Prior to processing a pre-authorization through the API, you must
			 * modify the transaction settings in your Beanstream merchant
			 * member area to allow for this transaction type. - Log in to the
			 * Beanstream online member area at
			 * www.beanstream.com/admin/sDefault.asp. - Navigate to
			 * administration - account admin - order settings in the left menu.
			 * Under the heading �Restrict Internet Transaction Processing
			 * Types,� select either of the last two options. The �Purchases
			 * or Pre-Authorization Only� option will allow you to process
			 * both types of transaction through your web interface.
			 * De-selecting the �Restrict Internet Transaction Processing
			 * Types� checkbox will allow you to process all types of
			 * transactions including returns, voids and pre-auth completions.
			 *
			 * capture (PAC) -> requires trnId -------------
			 *
			 * refund (R) ------------- REQUEST ->
			 * merchant_id=123456789&requestType=BACKEND&trnType=R&username=user1234&password=pass1234&trnOrderNumber=1234&trnAmount=1.00&adjId=10002115
			 * RESPONSE->
			 ...
			 *
			 *
			 * //notes //On receipt of the transaction response, the merchant
			 * must display order amount, transaction ID number, bank
			 * authorization code (authCode), currency, date and
			 * �messageText� to the customer on a confirmation page.
			 */
	...
	private Transaction parseResponse(TransactionEnum transactionType, PaymentEnum paymentType, Map<String, String> nvp,
		...
		transaction.setPaymentType(PaymentEnum.CREDITCARD);
