...
public class PayPalRestPayment implements PaymentModule {
	...
	public Transaction authorize(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
		...
		/*
		 * // ###AccessToken // Retrieve the access token from //
		 * OAuthTokenCredential by passing in // ClientID and ClientSecret
		 * APIContext apiContext = null; String accessToken = null;
		 *
		 * try {
		 *
		 ...
		 *
		 * // ### Api Context // Pass in a `ApiContext` object to authenticate
		 * // the call and to send a unique request id // (that ensures
		 * idempotency). The SDK generates // a request id if you do not pass
		 * one explicitly. apiContext = new APIContext(accessToken); // Use this
		 * variant if you want to pass in a request id // that is meaningful in
		 * your application, ideally // a order id.
		 *
		 * String requestId = Long.toString(System.nanoTime(); APIContext
		 * apiContext = new APIContext(accessToken, requestId ));
		 *
		 *
		 * // ###Authorization // Retrieve an Authorization Id // by making a
		 * Payment with intent // as 'authorize' and parsing through // the
		 * Payment object
		 *
		 *
		 ...
		 * //Details details = new Details(); //details.setShipping("0.03");
		 * //details.setSubtotal("107.41"); //details.setTax("0.03");
		 *
		 * // ###Amount // Let's you specify a payment amount.
		 *
		 * String sAmount = productPriceUtils.getAdminFormatedAmount(store,
		 * amount);
		 *
		 *
		 * Amount amnt = new Amount();
		 * amnt.setCurrency(store.getCurrency().getCode());
		 * amnt.setTotal(sAmount); //amnt.setDetails(details);
		 *
		 * // ###Transaction // A transaction defines the contract of a //
		 * payment - what is the payment for and who // is fulfilling it.
		 * Transaction is created with // a `Payee` and `Amount` types
		 * com.paypal.api.payments.Transaction transaction = new
		 * com.paypal.api.payments.Transaction(); transaction.setAmount(amnt);
		 * //TODO change description transaction.
		 * setDescription("This is the payment transaction description.");
		 *
		 * // The Payment creation API requires a list of // Transaction; add
		 * the created `Transaction` // to a List
		 * List<com.paypal.api.payments.Transaction> transactions = new
		 * ArrayList<com.paypal.api.payments.Transaction>();
		 * transactions.add(transaction);
		 *
		 * // ###Payer // A resource representing a Payer that funds a payment
		 * // Payment Method // as 'paypal' Payer payer = new Payer();
		 * payer.setPaymentMethod("paypal");
		 *
		 * // ###Payment // A Payment Resource; create one using // the above
		 * types and intent as 'sale' com.paypal.api.payments.Payment ppayment =
		 * new com.paypal.api.payments.Payment(); ppayment.setIntent("sale");
		 * ppayment.setPayer(payer); ppayment.setTransactions(transactions);
		 *
		 ...
		 * " and status = " + createdPayment.getState()); // ###Payment Approval
		 * Url Iterator<Links> links = createdPayment.getLinks().iterator();
		 ...
		 * map.put(guid, createdPayment.getId()); } catch (PayPalRESTException
		 * e) { req.setAttribute("error", e.getMessage()); } } catch
		 * (PayPalRESTException e) { throw new IntegrationException(e); }
		 */
	 ...
	 * IntegrationModule module) throws IntegrationException { // TODO
