...
import com.smartshop.core.catelog.service.PricingService;
...
import com.smartshop.eshop.common.PaymentConstants;
...
import com.smartshop.payment.dto.PaypalPaymentDTO;
...
public class PayPalExpressCheckoutPayment implements PaymentModule {
	...
	public Transaction authorize(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
		...
		PaypalPaymentDTO paypalPayment = (PaypalPaymentDTO) payment;
	 ...
	 *
	 * com.salesmanager.core.business.payments.model.PaypalPayment paypalPayment
	 * = (com.salesmanager.core.business.payments.model.PaypalPayment)payment;
	 * Validate.notNull(paypalPayment.getPaymentToken(),
	 * "A paypal payment token is required to process this transaction");
	 *
	 * return processTransaction(store, customer, items, amount, paypalPayment,
	 * configuration, module);
	 *
	 * }
	 */

	public Transaction initPaypalTransaction(MerchantStore store, List<ShoppingCartItem> items,
			OrderTotalSummary summary, PaymentDTO payment, IntegrationConfiguration configuration,
			IntegrationModule module) throws IntegrationException {
		...
		try {
			...
			if (configuration.getIntegrationKeys().get("transaction")
					.equalsIgnoreCase(TransactionEnum.AUTHORIZECAPTURE.name())) {
			...
			for (ShoppingCartItem cartItem : items) {
				...
				amt.setCurrencyID(
						urn.ebay.apis.eBLBaseComponents.CurrencyCodeType.fromValue(payment.getCurrency().getCode()));
				...
				item.setAmount(amt);
				// System.out.println(pricingService.getStringAmount(cartItem.getSubTotal(),
				// store));
				lineItems.add(item);
			...
			for (OrderTotal total : orderTotals) {
				...
				if (total.getModule().equals(BusinessConstants.OT_SHIPPING_MODULE_CODE)) {
					BasicAmountType shipping = new BasicAmountType();
					shipping.setCurrencyID(
							urn.ebay.apis.eBLBaseComponents.CurrencyCodeType.fromValue(store.getCurrency().getCode()));
					shipping.setValue(pricingService.getStringAmount(total.getValue(), store));
					// System.out.println(pricingService.getStringAmount(total.getValue(),
					// store));
					paymentDetails.setShippingTotal(shipping);
				}

				if (total.getModule().equals(BusinessConstants.OT_HANDLING_MODULE_CODE)) {
					BasicAmountType handling = new BasicAmountType();
					handling.setCurrencyID(
							urn.ebay.apis.eBLBaseComponents.CurrencyCodeType.fromValue(store.getCurrency().getCode()));
				...
				if (total.getModule().equals(BusinessConstants.OT_TAX_MODULE_CODE)) {
					if (tax == null) {
			...
			// System.out.println(pricingService.getStringAmount(itemsTotal,
			// store));
			paymentDetails.setOrderTotal(orderTotal);
			List<PaymentDetailsType> paymentDetailsList = new ArrayList<PaymentDetailsType>();
			paymentDetailsList.add(paymentDetails);

			StringBuilder RETURN_URL = new StringBuilder().append(coreConfiguration.getProperty("SHOP_SCHEME", "http"))
					.append("://").append(store.getDomainName()).append("/")
					.append(coreConfiguration.getProperty("CONTEXT_PATH", "sm-shop"));

			SetExpressCheckoutRequestDetailsType setExpressCheckoutRequestDetails = new SetExpressCheckoutRequestDetailsType();
			String returnUrl = RETURN_URL.toString() + new StringBuilder().append(PaymentConstants.SHOP_URI)
					.append("/paypal/checkout").append(coreConfiguration.getProperty("URL_EXTENSION", ".html"))
					.append("/success").toString();
			String cancelUrl = RETURN_URL.toString() + new StringBuilder().append(PaymentConstants.SHOP_URI)
					.append("/paypal/checkout").append(coreConfiguration.getProperty("URL_EXTENSION", ".html"))
			...
			if (BusinessConstants.PRODUCTION_ENVIRONMENT.equals(env)) {
				mode = "production";
			}
			...
			transaction.setTransactionType(TransactionEnum.INIT);
			transaction.setPaymentType(PaymentEnum.PAYPAL);
	...
	public Transaction refund(boolean partial, MerchantStore store, Transaction transaction, SalesOrder order,
		...
		try {
			...
			Validate.notNull(transaction.getTransactionDetails().get("TRANSACTIONID"),
			...
			refundAmount.setCurrencyID(
					urn.ebay.apis.eBLBaseComponents.CurrencyCodeType.fromValue(order.getCurrency().getCode()));
			...
			if (!"Success".equals(refundAck)) {
				...
				throw new IntegrationException(BusinessException.EXCEPTION_TRANSACTION_DECLINED,
						"Paypal refund transaction code [" + refundTransactionResponse.getErrors().get(0).getErrorCode()
								+ "], message-> " + refundTransactionResponse.getErrors().get(0).getShortMessage());
			}

			Transaction newTransaction = new Transaction();
			newTransaction.setAmount(amount);
			newTransaction.setTransactionDate(new Date());
			newTransaction.setTransactionType(TransactionEnum.REFUND);
			newTransaction.setPaymentType(PaymentEnum.PAYPAL);
	...
	private Transaction processTransaction(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
		...
		try {
			...
			// TOKEN=EC-9VT64354BS889423P&CHECKOUTSTATUS=PaymentActionNotInitiated&TIMESTAMP=2014-01-26T17:30:17Z&CORRELATIONID=84dfe1d0939cc&ACK=Success&VERSION=104.0&BUILD=9285531&EMAIL=csamson777-facilitator@yahoo.com&PAYERID=XURV79Z6URDV4&PAYERSTATUS=verified&BUSINESS=facilitator
			// account's Test
			// Store&FIRSTNAME=facilitator&LASTNAME=account&COUNTRYCODE=US&SHIPTONAME=facilitator
			// account's Test Store&SHIPTOSTREET=1 Main St&SHIPTOCITY=San
			// Jose&SHIPTOSTATE=CA&SHIPTOZIP=95131&SHIPTOCOUNTRYCODE=US&SHIPTOCOUNTRYNAME=United
			// States&ADDRESSSTATUS=Confirmed&CURRENCYCODE=USD&AMT=1.00&ITEMAMT=1.00&SHIPPINGAMT=0.00&HANDLINGAMT=0.00&TAXAMT=0.00&INSURANCEAMT=0.00&SHIPDISCAMT=0.00&L_NAME0=item&L_QTY0=1&L_TAXAMT0=0.00&L_AMT0=1.00&L_ITEMWEIGHTVALUE0=
			// 0.00000&L_ITEMLENGTHVALUE0= 0.00000&L_ITEMWIDTHVALUE0=
			// 0.00000&L_ITEMHEIGHTVALUE0=
			// 0.00000&PAYMENTREQUEST_0_CURRENCYCODE=USD&PAYMENTREQUEST_0_AMT=1.00&PAYMENTREQUEST_0_ITEMAMT=1.00&PAYMENTREQUEST_0_SHIPPINGAMT=0.00&PAYMENTREQUEST_0_HANDLINGAMT=0.00&PAYMENTREQUEST_0_TAXAMT=0.00&PAYMENTREQUEST_0_INSURANCEAMT=0.00&PAYMENTREQUEST_0_SHIPDISCAMT=0.00&PAYMENTREQUEST_0_INSURANCEOPTIONOFFERED=false&PAYMENTREQUEST_0_SHIPTONAME=facilitator
			// account's Test Store&PAYMENTREQUEST_0_SHIPTOSTREET=1 Main
			...
			// States&PAYMENTREQUEST_0_ADDRESSSTATUS=Confirmed&PAYMENTREQUEST_0_ADDRESSNORMALIZATIONSTATUS=None&L_PAYMENTREQUEST_0_NAME0=item&L_PAYMENTREQUEST_0_QTY0=1&L_PAYMENTREQUEST_0_TAXAMT0=0.00&L_PAYMENTREQUEST_0_AMT0=1.00&L_PAYMENTREQUEST_0_ITEMWEIGHTVALUE0=
			// 0.00000&L_PAYMENTREQUEST_0_ITEMLENGTHVALUE0=
			// 0.00000&L_PAYMENTREQUEST_0_ITEMWIDTHVALUE0=
			// 0.00000&L_PAYMENTREQUEST_0_ITEMHEIGHTVALUE0=
			...
			/** sale or pre-auth **/
			if (payment.getTransactionType().name().equals(TransactionEnum.AUTHORIZE.name())) {
	...
	public Transaction capture(MerchantStore store, Customer customer, SalesOrder order,
		...
		try {
			...
			Validate.notNull(capturableTransaction.getTransactionDetails().get("TRANSACTIONID"),
					"Transaction details must contain a TRANSACTIONID");
			Validate.notNull(order, "Order must not be null");
			Validate.notNull(order.getCurrency(), "Order nust contain Currency object");
			...
			DoCaptureRequestType doCaptureRequest = new DoCaptureRequestType(
					capturableTransaction.getTransactionDetails().get("TRANSACTIONID"), amount,
			...
			newTransaction.setTransactionDate(new Date());
			newTransaction.setTransactionType(TransactionEnum.CAPTURE);
			newTransaction.setPaymentType(PaymentEnum.PAYPAL);
