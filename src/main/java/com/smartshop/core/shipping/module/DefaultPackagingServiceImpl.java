...
@Service("boxPackaging")
public class DefaultPackagingServiceImpl implements Packaging {
	...
	public List<PackageDetails> getBoxPackagesDetails(List<ShippingProduct> products, MerchantStore store)
		...
		if (products == null) {
			throw new BusinessException("Product list cannot be null !!");
		}
		...
		if (shippingConfiguration == null) {
			throw new BusinessException("ShippingConfiguration not found for merchant " + store.getCode());
		}
		...
		if (maxVolume == 0 || maxweight == 0) {
			...
			merchantLogService.save(new MerchantLog(store, "shipping",
					"Check shipping box configuration, it has a volume of " + maxVolume + " and a maximum weight of "
							+ maxweight + ". Those values must be greater than 0."));

			throw new BusinessException("Product configuration exceeds box configuraton");
		...
		for (Product p : individualProducts) {
			...
			if (p.getProductWidth().doubleValue() > width || p.getProductHeight().doubleValue() > height
					|| p.getProductLength().doubleValue() > length) {
				// log message to customer
				merchantLogService.save(new MerchantLog(store, "shipping", "Product " + p.getSku()
			...
			double productVolume = (p.getProductWidth().doubleValue() * p.getProductHeight().doubleValue()
					* p.getProductLength().doubleValue());
			...
			for (PackingBox pbox : boxesList) {
				...
				if ((volumeLeft * .75) >= productVolume && pbox.getWeightLeft() >= productWeight) {// fit
																									// the
																									// item
																									// in
																									// this
