package com.smartshop.core.shipping.service.impl;
...
import com.smartshop.core.shipping.model.ShippingOptionPriceEnum;
import com.smartshop.core.shipping.model.ShippingPackageEnum;
...
public class ShippingServiceImpl implements ShippingService {
	...
	@Override
	public ShippingConfiguration getShippingConfiguration(MerchantStore store) throws BusinessException {
		...
		if (configuration != null) {
			...
			try {
				...
				throw new BusinessException("Cannot parse json string " + value);
			}
		}
	...
	@Override
	public void saveShippingConfiguration(ShippingConfiguration shippingConfiguration, MerchantStore store)
			throws BusinessException {
	...
	@Override
	public void saveCustomShippingConfiguration(String moduleCode, CustomIntegrationConfiguration shippingConfiguration,
			MerchantStore store) throws BusinessException {

		ShippingQuoteModule quoteModule = shippingModules.get(moduleCode);
		if (quoteModule == null) {
			throw new BusinessException("Shipping module " + moduleCode + " does not exist");
		}
	...
	@Override
	public void saveShippingQuoteModuleConfiguration(IntegrationConfiguration configuration, MerchantStore store)
			throws BusinessException {
		...
		try {
			...
			ShippingQuoteModule quoteModule = shippingModules.get(moduleCode);
			if (quoteModule == null) {
				throw new BusinessException("Shipping quote module " + moduleCode + " does not exist");
			}
	...
	@Override
	public ShippingQuote getShippingQuote(MerchantStore store, Delivery delivery, List<ShippingProduct> products,
		...
		// IntegrationModule -> The concrete module as defined in
		// integrationmodules.properties
		...
		try {
			...
			ShippingEnum shippingType = ShippingEnum.INTERNATIONAL;
			...
			if (shippingType.name().equals(ShippingEnum.NATIONAL.name())) {
			...
			} else if (shippingType.name().equals(ShippingEnum.INTERNATIONAL.name())) {
			...
			if (shippingConfiguration.isFreeShippingEnabled()) {
				...
				if (freeShippingAmount != null) {
					if (orderTotal.doubleValue() > freeShippingAmount.doubleValue()) {
						if (shippingConfiguration.getFreeShippingType() == ShippingEnum.NATIONAL) {
			...
			if (!CollectionUtils.isEmpty(shippingModulePreProcessors)) {
				for (ShippingQuotePrePostProcessModule preProcessor : shippingModulePreProcessors) {
					// System.out.println("Using pre-processor " +
					// preProcessor.getModuleCode());
					...
					if (shippingQuote.getCurrentShippingModule() != null
							&& !shippingQuote.getCurrentShippingModule().getCode().equals(shippingModule.getCode())) {
						shippingModule = shippingQuote.getCurrentShippingModule();// determines
																					// the
																					// shipping
																					// module
			...
			try {
				shippingOptions = shippingQuoteModule.getShippingQuotes(shippingQuote, packages, orderTotal, delivery,
						shippingOrigin, store, configuration, shippingModule, shippingConfiguration, locale);
			} catch (Exception e) {
				LOGGER.error("Error while calculating shipping : " + e.getMessage(), e);
				merchantLogService.save(new MerchantLog(store,
						"Can't process " + shippingModule.getModule() + " -> " + e.getMessage()));
			...
			if (shippingOptions == null && !StringUtils.isBlank(delivery.getPostalCode())) {
				...
				// absolutely need to use in this case store pickup or other
			...
			ShippingOptionPriceEnum shippingOptionPriceType = shippingConfiguration.getShippingOptionPriceType();
			...
			if (shippingOptions != null) {
				...
				for (ShippingOption option : shippingOptions) {
					...
					if (shippingOptionPriceType.name().equals(ShippingOptionPriceEnum.HIGHEST.name())) {
					...
					if (shippingOptionPriceType.name().equals(ShippingOptionPriceEnum.LEAST.name())) {
					...
					if (shippingOptionPriceType.name().equals(ShippingOptionPriceEnum.ALL.name())) {
				...
				if (selectedOption != null
						&& !shippingOptionPriceType.name().equals(ShippingOptionPriceEnum.ALL.name())) {
	...
	public List<String> getSupportedCountries(MerchantStore store) throws BusinessException {
		...
		MerchantConfiguration configuration = merchantConfigurationService.getMerchantConfiguration(SUPPORTED_COUNTRIES,
				store);
	...
	@Override
	public List<Country> getShipToCountryList(MerchantStore store, Language language) throws BusinessException {
	...
	@Override
	public List<PackageDetails> getPackagesDetails(List<ShippingProduct> products, MerchantStore store)
			throws BusinessException {
		...
		ShippingPackageEnum shippingPackageType = ShippingPackageEnum.ITEM;
		if (shippingConfiguration != null) {
			shippingPackageType = shippingConfiguration.getShippingPackageType();
		}

		if (shippingPackageType.name().equals(ShippingPackageEnum.BOX.name())) {
	...
	@Override
	public boolean requiresShipping(List<ShoppingCartItem> items, MerchantStore store) throws BusinessException {
	...
	@Override
	public ShippingMetaData getShippingMetaData(MerchantStore store) throws BusinessException {
