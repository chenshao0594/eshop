...
import { <%= entityAngularName %>PopupComponent, <%= entityAngularName %>DialogComponent } from './<%= entityFileName %>-dialog.component';
...
export const <%= entityInstance %>Route: Routes = [
  {
    ...
    path: '<%= entityUrl %>-new',
    component: <%= entityAngularName %>DialogComponent,
    data: {
        authorities: ['ROLE_USER'],
        pageTitle: <% if (enableTranslation){ %>'<%= angularAppName %>.<%= entityTranslationKey %>.home.title'<% }else{ %>'<%= entityClassPlural %>'<% } %>
    },
