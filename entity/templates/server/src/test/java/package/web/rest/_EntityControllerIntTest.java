...
<%_ } _%>
public class <%= entityClass %>ControllerIntTest <% if (databaseType == 'cassandra') { %>extends AbstractCassandraTest <% } %>{
<%_
    let oldSource = '';
    try {
        oldSource = this.fs.readFileSync(this.SERVER_TEST_SRC_DIR + packageFolder + '/web/rest/' + entityClass + 'ControllerIntTest.java', 'utf8');
    } catch (e) {}
_%>
    <%_ for (idx in fields) {
        ...
        if (!this._.isUndefined(fields[idx].fieldValidateRulesPattern)) {
            ...
            if (sampleTextString === updatedTextString) {
                ...
                log(this.chalkRed('Randomly generated first and second test values for entity "' + entityClass +
                    '" field "' + fields[idx].fieldName + '" with pattern "' + fields[idx].fieldValidateRulesPattern +
                    '" in file "' + entityClass + 'ControllerIntTest" where equal, added symbol "B" to second value.'));
            }
        }_%>
...
<%_ } _%>
    ...
    public void setup() {
        ...
        <%_ if (service != 'no') { _%>
        <%= entityClass %>Controller <%= entityInstance %>Controller = new <%= entityClass %>Controller(<%= entityInstance %>Service);
        <%_ } else { _%>
        <%= entityClass %>Controller <%= entityInstance %>Controller = new <%= entityClass %>Controller(<%= entityInstance %>Repository<% if (dto == 'mapstruct') { %>, <%= entityInstance %>Mapper<% } %><% if (searchEngine == 'elasticsearch') { %>, <%= entityInstance %>SearchRepository<% } %>);
        <%_ } _%>
        this.rest<%= entityClass %>MockMvc = MockMvcBuilders.standaloneSetup(<%= entityInstance %>Controller)
    ...
    public static <%= entityClass %> createEntity(<% if (databaseType == 'sql') { %>EntityManager em<% } %>) {
        ...
        <%_ for (idx in relationships) {
            ...
            if (relationshipValidate != null && relationshipValidate === true) { _%>
        // Add required entity
        <%= otherEntityNameCapitalized %> <%= relationshipFieldName %> = <%= otherEntityNameCapitalized %>ControllerIntTest.createEntity(em);
